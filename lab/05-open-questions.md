...
|---|---|---|---|
| 1 | Is `<firewall-mgmt-if>` OPNsense or another router? | OPNsense `em2` (OPT2) | `00` §3 |
| 2 | What is `<firewall-uplink>`? | OPNsense `em0` (LAN/uplink) — **not** a sinkhole | `00` §3 |
...
| 6 | Why no ping reply from `analysis-host`? | VM running, pf permits it → Windows Firewall ICMP | `02` §5 |
...
| 10 | Defender on `analysis-host`? | **Disabled by Elastic Security** — Defend is the sole EDR | `02` §5 |
| 18 | OPNsense GUI location? | **`http://<firewall-detonation-if>`** — plain HTTP, port 80 | `01` §1 |
...
1. ~~`analysis-host` has not shipped telemetry~~ — **not a fault**: the host is powered down
...
4. **OPNsense GUI is HTTP-only** — a TLS cert reference already exists; enabling HTTPS
   is a config toggle.
5. **The detonation LAN can reach the utility LAN** — a compromised `analysis-host` could
   reach collector, which runs the CTI app, the log relay, and the tunnel.
